...
async function createBundle(): Promise<void> {
  temporaryDirectory = await mkdtemp(join(tmpdir(), 'kakomare-quality-a-'));
  const repositoryRoot = join(import.meta.dirname, '..', '..');
  const output = join(temporaryDirectory, 'battle.mjs');
  await build({
    stdin: {
      contents: `export { BattleScene } from ${JSON.stringify(join(repositoryRoot, 'src/game/scenes/BattleScene.ts'))}; export { Enemy } from ${JSON.stringify(join(repositoryRoot, 'src/game/entities/Enemy.ts'))};`,
...
describe('BattleScene の実戦処理を使うPR-A回帰', () => {
