...
- 実装状態、検査状態、公開受入状態は別に管理します。V5時点では実装済み${implementedWeaponCount}、公開受入済み0です。
`;

const supports = `# 補助カタログ（V5実装正本）

補助総数Sは **20** と決定します。既存6件だけでは、50武器へ攻撃変更・条件発動・接続・防衛・代償を分担させる台帳が不足するため14件を追加しました。V5時点で戦闘に登録されているのは${implementedSupportCount}件です。各補助は実効値の上限、非対応理由、接続条件を持ち、候補から到達できることを検査します。
...
const matrix = `# 武器×補助適用マトリクス（V5実装正本）
...
V5では全50武器・全20補助を戦闘登録へ移し、個体・上限・候補到達を検査しました。V6では保存v3、途中復帰、結果精算、ゲーム側ランキング通信の準備を行い、長時間本戦・公開受入はV7へ残します。
`;

const rules = `# 競技型無限モードのルール契約（V6ゲーム側準備済み／本番未接続）

この文書は、後続工程が同じ条件を参照するための設計契約です。\`${COMPETITIVE_RULES.version}\` は本番ランキングを有効化する識別子ではありません。V6ではゲーム側の保存・結果・送信アダプターとmanifestを準備しましたが、実験場の実際の登録、RPC署名・返り値・認証・権限は未確認です。既定値は未設定ゲートウェイで、本番データベースや実験場設定へ接続・変更しません。
...
V6のゲーム側は、開始再送用の\`start_id\`、受付が発行する\`play_id\`、結果送信用の一つの\`submission_id\`、finish→submitの順序、応答喪失後の同一内容再送、idle／submitting／submitted／retryable_failed／permanent_failedを実装しています。結果・精算台帳は一つの\`resultId\`を一回だけ反映し、リタイアをランキング対象へ送りません。\`ranking-manifest.json\`の正式URL・game_id・版・採点順序を検証します。
...
const balance = `# V6へ引き継いだ設計判断と未測定範囲
...
if (mismatches.length > 0) {
  ...
  console.log('V6拡張文書を確認しました（5ファイル）。');
} else {
  console.log('V6拡張文書を生成しました（5ファイル）。');
}
