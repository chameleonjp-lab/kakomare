...
V5では全50武器・全20補助を戦闘登録へ移し、個体・上限・候補到達を検査します。長時間本戦、保存v3、ランキング通信、公開受入はV6〜V7で行います。
`;

const rules = `# 競技型無限モードのルール契約（V5引き継ぎ／V6実装待ち）

この文書は、後続工程が同じ条件を参照するための設計契約です。\`${COMPETITIVE_RULES.version}\` は本番ランキングを有効化する識別子ではありません。V5では本番データベース、開始受付、送信処理を変更しません。V6で共通規約の実際の署名・返り値・権限を照合し、別許可を得てから接続します。
...
V5ではこの契約と型・検証器を引き継ぎます。ランキングの開始受付、結果送信、再送、認証、本番登録、途中保存の資格はV6の対象です。本番DBや実験場の設定をこの工程で変更しません。採点係数はV1の比較案を出発点とし、V4以後の本戦データで変更する場合は旧値・新値・理由・版を残します。
`;

const balance = `# V5実装に引き継いだ設計判断と未測定範囲
...
if (mismatches.length > 0) {
  ...
  console.log('V5拡張文書を確認しました（5ファイル）。');
} else {
  console.log('V5拡張文書を生成しました（5ファイル）。');
}
