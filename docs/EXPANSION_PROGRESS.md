...
更新日：2026-09-16（UTC）。[計画v2.0](EXPANSION_IMPLEMENTATION_PLAN.md) に従い、このファイルを唯一の進行正本とする。旧PR-1の変更・失敗・成功記録は [履歴](history/EXPANSION_PROGRESS_PR16.md) に保存し、現在の成功判定へ流用しない。

## 現在の作業：強化候補の選択境界と端末別表示を揃える（2026-09-16 JST）

- PR #36で強化候補の説明階層を整理した後も、候補カード全体が選択操作に見えやすく、候補名のボタン以外を押したときの境界が画面から分かりにくかった。候補名のボタンと装着場所のボタンだけを操作対象として明示し、候補名ボタンを内容に合わせた幅へ戻す。
- 端末判定はタッチ能力・ポインター・画面幅を使い、スマートフォン／タブレットをモバイル表示、マウス中心の広い画面をデスクトップ表示としてルートへ記録する。判定はゲーム規則・戦闘計算・候補抽選を変更しない。
- 広いPC画面では戦場を600pxの固定表示枠、右側の操作情報を320pxとして揃える。1280pxと1920pxの画面で同じ表示寸法を使い、狭いウィンドウとモバイルでは既存の安全な縦積み・横画面配置を維持する。
- 追加する回帰検査は、説明部分のタップで候補が確定しないこと、選択ボタンの幅がカード全体より狭いこと、端末プロファイルの切替、広いPC画面の固定戦場サイズである。iPhone Safari実機の最終確認は別途必要とする。


## 現在の作業：Lv10以降の敵数と強化画面の停止・復帰を安定させる（2026-09-13 JST）

- PR #29は2026-09-13T11:56:01Zにマージ済み。PR #30もマージ済みで、現在のmainはマージコミット `5896ecca1853ff80b217be7e5aea4be255ae4824`。スマホ幅の強化候補内で、配置・効果・組み合わせを1列で読める状態を基準にする。
- 調査で、Lv10以降も敵の生成予算がプレイヤーの成長に連動していなかったため、Lv10から段階的に生成予算を増やし、敵が少なく感じる区間を埋める。通常の同時敵上限、敵の種類、ボス、端末負荷による難易度は変更しない。
- 強化画面はモーダル全体を唯一のスクロール領域とし、内部ダイアログの二重スクロールを解消する。長い効果説明・組み合わせ説明でも最下部へ到達できることを幅390pxの画面検査で確認する。
- 強化候補を生成した直後、引き直し・除外後にも安全境界を保存する。ブラウザ再読込後は一時停止から明示的に再開し、未選択候補を表示したまま選択を待つ。
- `tests/unit/spawn-and-simulation.test.ts` にLv10以降の敵供給回帰、`tests/e2e/core.spec.ts` に強化画面の最下部スクロールと再読込復帰の回帰を追加した。Quality #88（run `34765218725`）は、静的・型・単体、Chromium、WebKit、build/配布物検査、V7 final gatesの全job成功を確認した。Quality #87は任意引数後の必須引数エラーで失敗したが、同じbranchで修正した。
- CIは成功したが、iPhone Safari実機、VoiceOver、片手操作、発熱、独立Sol・Highレビューは未実施。最終的なマージと実機確認はユーザーの受入範囲とする。
- 実装組織図の標準形に従い、設計判断・受入条件はAstra・High、日常実装と複雑な検査はLuna・Max、公開前の独立レビューはSol・Highの確認対象とする。
- 変更境界は、敵生成予算、強化候補モーダルのスクロール、安全境界保存、回帰検査、進行記録に限定する。本番ランキング、DB、実験場の受付、既存セーブの破棄、main保護設定は変更しない。
