...
- 未確認：V7 branchのDraft PR／提出commitに対するGitHub Actions、ローカルPlaywright実行ファイルを用いたChromium／WebKit本体（この環境では実行ファイル不在）、iPhone Safari・VoiceOver・片手操作・発熱、独立Sol・Highレビュー、実験場のRPC署名・登録値・認証・権限・受付側再計算、本番ランキング有効化。これらを自動試行成功へ繰り上げない。
...
Draft PRはこの差分の検査・文書更新後に作成し、提出commitとActions URLを本節へ追記する。ユーザーのマージ後にのみ公開版・実機・本番受付を再確認する。
