...
## 現在の作業：蓄勢環の表示説明と生成カタログの同期（2026-09-13 JST）

- PR #28は外部操作により2026-09-13T09:11:46Zにマージ済み。現在のmainはマージコミット `d58c696f9b7b1c1e0042ff9670bb455a9059be56` で、蓄勢環の実効発射間隔・単体検査・BattleScene結合検査が反映されている。
- PR #28のQuality #82はマージ前コミットの起動失敗で、検査成功とは扱わない。表示文言と生成カタログの未反映分を、このmainから `codex/reserve-display-sync-20260913` として切り出した。
- Quality #83では、追加した実効間隔の下限 `0.25` が蓄勢環に限らず全武器へ掛かり、既存 `v2-foundation` の短間隔武器（期待値0.16秒）を壊した。33ファイル中1テスト失敗、245テスト成功を確認し、全武器の既存間隔を保つ修正をPR #29先端へ反映した。マージ後mainのQuality #30も同じ回帰で失敗している。
- `supports.ts`、`EquipmentHelp.ts`、`expansionCatalog.ts`、`SUPPORT_CATALOG.md` の蓄勢環表記を「再発射間隔」へ同期し、ため時間の下限0.8秒と、ほかの武器の蓄積・放出へ働かない制約を同じ文言で保つ。
- `runtimeState.lanceCharge` の保存・復元と、PR #28で追加した実効値・発射時刻の回帰検査は維持する。今回も既存セーブの破棄や保存形式変更は行わない。
- 検査状態：表示同期と生成文書の差分を後続ブランチへ反映中。lint、typecheck、単体・結合、生成文書、E2E、build、配布物検査、独立レビューは未完了であり、成功扱いにしない。
- 実装組織図の標準形に従い、設計判断・受入条件はAstra・High、日常実装と複雑な検査はLuna・Max、公開前の独立レビューはSol・Highの確認対象とする。
- 変更境界は、蓄圧槍の表示説明・設計台帳・生成カタログ・進行記録に限定する。本番ランキング、DB、実験場の受付、既存セーブの破棄、main保護設定は変更しない。
