...
## 現在の作業：強化候補の「この場所で働く効果・組み合わせ」をスマホ幅で読みやすくする（2026-09-13 JST）

- PR #29は2026-09-13T11:56:01Zにマージ済み。現在のmainはマージコミット `d05ab802ca477ce4f54a948ec032d1cbfa2a6cb9` で、蓄勢環の実効発射間隔と表示説明の同期が反映されている。
- 添付画像を確認し、`@media (max-width: 560px)` の `.upgrade-placement-list` が3列になっていたため、配置ボタン・接続説明・効果詳細が1つの配置単位ではなく横方向へ分割され、効果・組み合わせの文が極端に狭い列で折り返されることを特定した。
- 後続ブランチ `followup/upgrade-detail-readability-20260913` で、配置候補の操作・接続説明・`details` を1列に戻し、スマホ幅でも効果・組み合わせの本文が候補カード幅を使えるようにする。ランタイム計算、保存形式、装着処理は変更しない。
- 受入条件は、幅390pxの候補画面で効果詳細が候補カード幅の70%以上を使い、配置ボタンが詳細本文の高さへ引き伸ばされず、横スクロールを発生させないこと。既存の「一度のタップで装着」経路は維持する。
- `tests/e2e/core.spec.ts` の満枠武器交換検査へ、効果詳細の幅と配置ボタンの高さの回帰確認を追加した。lint、typecheck、単体・結合、E2E、build、配布物検査、独立レビューは未完了であり、成功扱いにしない。
- 実装組織図の標準形に従い、設計判断・受入条件はAstra・High、日常実装と複雑な検査はLuna・Max、公開前の独立レビューはSol・Highの確認対象とする。
- 変更境界は、強化候補モーダルのスマホ向けレイアウト、E2E回帰検査、進行記録に限定する。本番ランキング、DB、実験場の受付、既存セーブの破棄、main保護設定は変更しない。
