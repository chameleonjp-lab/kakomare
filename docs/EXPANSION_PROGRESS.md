...
## 現在の作業：蓄勢環の定常発射短縮を実処理へ接続（2026-09-13 JST）

- PR #27は2026-09-12T21:49:24Zにマージ済み。現在のmainはマージコミット `785c5c2103ad40fa563c12d9793840af8f90ea5e`。mainへ直接push・マージ・自動マージは行わず、このmainから後続ブランチ `followup/reserve-effective-cadence` を作成した。
- PR #27後の追加監査で、蓄勢環の短縮が `fireLance` のため時間判定には入るものの、レベル1〜3の下限0.8秒に吸収され、`Weapon.advance` が使う定常発射間隔へ届いていないことを確認した。ため時間下限は維持し、蓄圧槍だけの実効発射間隔へ短縮を接続する。
- 実装対象は `effectiveWeaponStats` と本番 `BattleScene.step` の発射経路。律動環の短縮とは別に、蓄勢環のレベル1〜3を0.08／0.16／0.24秒として適用し、他の武器の間隔・蓄積・放出は変えない。
- `runtimeState.lanceCharge` は既にチェックポイントへ保存・復元されているため、今回の保存形式変更は行わない。置換・途中復帰の既存回帰を維持し、今回の実処理短縮の単体・結合検査を追加する。
- 検査状態：コードと回帰検査を後続ブランチへ反映中。lint、typecheck、単体・結合、生成文書、E2E、build、配布物検査、独立レビューは未完了であり、成功扱いにしない。
- 実装組織図の標準形に従い、設計判断・受入条件はAstra・High、日常実装と複雑な検査はLuna・Max、公開前の独立レビューはSol・Highの確認対象とする。
- 変更境界は、蓄圧槍の実効発射間隔・表示説明・回帰検査・進行記録に限定する。本番ランキング、DB、実験場の受付、既存セーブの破棄、main保護設定は変更しない。
