...
const RUNTIME_WEAPON_NOTES: Record<WeaponId, RuntimeWeaponNote> = {
  ...
  lance: {
    ...
    condition: '破槍型は威力と貫通数を増やし、蓄勢環があれば定常発射間隔を少し短くします。',
    limits: ['ため時間には0.8秒の下限があります。蓄勢環は定常発射間隔を短くします。長槍型は射程を18%、幅を4広げ、破槍型は貫通数を増やします。'],
  },
  drone: {
...
function supportValueText(id: SupportId, module: SupportModule, total: { primary: number; secondary: number }): string {
  switch (id) {
    ...
    case 'reserve': return `蓄圧槍の再発射間隔を最大${number(Math.min(0.24, total.primary * 0.08))}秒短縮`;
...
function supportApplicability(weaponId: WeaponId, supportId: SupportId, weapon: WeaponSnapshot | undefined): string | null {
  switch (supportId) {
    ...
    case 'reserve': return weaponId === 'lance' ? '蓄圧槍の再発射間隔に働きます。' : null;
...
function supportHelp(input: SupportId | SupportSnapshot, weapons: readonly WeaponSnapshot[]): EquipmentHelp {
  ...
  switch (id) {
    ...
    case 'reserve': conditions.push('蓄圧槍の次の発射を待つとき。'); limits.push('ため時間の下限は0.8秒です。短縮は定常発射間隔へ反映されます。'); break;
